...
<link rel="stylesheet" type="text/css" href="{{ asset('public/frontend/styles/shop_responsive.css') }}">
	...
	<div class="home">
		...
		<div class="home_content d-flex flex-column align-items-center justify-content-center">
			<h2 class="home_title">
				@php
					$OneCatName  = DB::table('categories')->where('id',$singleProduct->category_id)->first();
					$subCatName  = DB::table('sub_categories')->where('id',$singleProduct->subcategory_id)->first();
				@endphp
				{{ $OneCatName->category_name }} > {{ $subCatName->subcategory_name }}
			</h2>
		</div>
	</div>
					...
					<div class="shop_sidebar">
						...
						<div class="sidebar_section">
							...
							<ul class="brands_list">
                                ...
                                @php
                                    $brandName = DB::table('brands')->where('id',$row->brand_id)->first();
                                @endphp
								<li class="brand"><a href="#">{{ $brandName->name }}</a></li>                                    
                                @endforeach
							</ul>
						</div>
					</div>
